 ...
 * 一个activity，用户可以做一些独立的，有焦点的事。几乎所有的activity都是用来和用户进行交互的
 * 所有activity会维护一个窗口，用来放置开发者的UI布局，通过setContentView（）来设置。
 * activity通常以一个屏幕窗口的方式展示出来，但也可以有其他使用方式，作为一个浮动的窗口（
 * 通过设置一个主题参数windowIsFloating来设置）或者嵌入其他的activity（使用ActivityGroup）
 *
 * There are two methods almost all subclasses of Activity will implement:
 * 有两个方法几乎是所有的activity都要实现的：
 *
 * <ul>
 ...
 *     programmatically.
 *     onCreate是在初始化activity时用的。更重要的是，你通常会在此方法里调用setContentView(int)（参数是一个布局文件）
 *     来确定UI，通过findViewById来重新获得UI中的组件，通过这些组件来处理一些事件
 *
 ...
 *     onPause方法会在离开activity的时候被调用。许多改变需要在这个环节来提交（例如：用contentProvider来吧数据保存起来）
 * </ul>
 *
 ...
 * 为了开发者能通过{android.content.Context#startActivity Context.startActivity()}来调用activity，所有的
 * activity都必须在清单文件中声明
 *
 * <p>Topics covered here:
 * 这里有一些关注点需要注意：
 * <ol>
 * <li><a href="#Fragments">Fragments</a>//fragment
 * <li><a href="#ActivityLifecycle">Activity Lifecycle</a>//activity生命周期
 * <li><a href="#ConfigurationChanges">Configuration Changes</a>//参数改变
 * <li><a href="#StartingActivities">Starting Activities and Getting Results</a>//打开一个新的activity，并获得结果
 * <li><a href="#SavingPersistentState">Saving Persistent State</a>//保存需要的状态
 * <li><a href="#Permissions">Permissions</a>//权限
 * <li><a href="#ProcessLifecycle">Process Lifecycle</a>//进程的生命周期
 * </ol>
 *
...
public class Activity extends ContextThemeWrapper
    ...
    如果activity通过了一个短暂的暂停，这个值为true
    /*package*/ boolean mTemporaryPause = false;
    /** true if the activity is being destroyed in order to recreate it with a new configuration */
    如果一个activity为了配置一些新的参数而被摧毁重建，则为true
    ...
    在FragmentController被构造之前，我们必须有一个handler
    final Handler mHandler = new Handler();
    final FragmentController mFragments = FragmentController.createController(new HostCallbacks());

    // Most recent call to requestVisibleBehind().
    最近调用了requestVisibleBehind()
     ...
     * 改变通过getIntent()返回的意图，这个保留了新设置的意图；不是把设置的意图复制一个出来保存
     * 通常接下来使用的方法是（onNewIntent）
     *
    ...
    返回，这个activity属于的application
    public final Application getApplication() {
        return mApplication;
    }

    /** Is this activity embedded inside of another activity? */
    这个activity是否有放到其他的activity之下
    public final boolean isChild() {
        return mParent != null;
    }

    /** Return the parent activity if this view is an embedded child. */
    如果是嵌入其他activity，返回这个activity
    public final Activity getParent() {
        return mParent;
    }

    /** Retrieve the window manager for showing custom windows. */
    重新获得windowManager来展示一个自定义的windows
     ...
     * 获得当前activity的窗口。这样可以用来直接获取一些跟activity和屏幕无关的窗口的参数，
     *
     * @return Window The current window, or null if the activity is not
     *         visual.
     *         activity不可见，则获得的窗口为空
     */
